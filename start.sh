...
# URL变量
SUBCONVERTER_DOWNLOAD_URL="https://ghfast.top/github.com/tindy2013/subconverter/releases/latest/download/subconverter_linux64.tar.gz"
...
download_clash() {
    local arch=$1
    local url="https://ghfast.top/github.com/MetaCubeX/mihomo/releases/download/${CLASH_VERSION}/mihomo-linux-${arch}-${CLASH_VERSION}.gz"
...
install_yq() {
    ...
    for attempt in $(seq 1 $MAX_RETRIES); do
        if $WGET_CMD \
            ...
            --retry-connrefused --waitretry=1 --read-timeout=20 --timeout=15 -t 3 \
            -O "$YQ_BINARY" \
            "https://ghfast.top/github.com/mikefarah/yq/releases/download/${YQ_VERSION}/yq_linux_amd64"; then
            if [ -f "$YQ_BINARY" ]; then
                chmod +x "$YQ_BINARY"
